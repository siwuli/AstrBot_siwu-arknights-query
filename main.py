...
5.【图片直发、文字自组织】资料图片会由查询工具直接发送给用户（你无法看到图片内容），
   工具返回的"图片已发送"摘要只是告诉你图片已经发过去了；收到后只需一句话告诉用户
   "图片已发送、请直接查看上方图片"即可收尾，严禁复述/转述/编造图片中的任何具体数值或材料清单，
   不要重复发送图片，不要调用 send_message_to_user 等通用发消息工具。
...
def _image_sent_text(subject: str) -> str:
    """资料图片直发后返回给 Agent 的简短摘要：图片已直发、让用户直接看图，Agent 只需一句话收尾。"""
    return (
        f"已向用户直接发送「{subject}」的资料图片，图片已展示在用户面前，用户现在可直接查看。"
        "你无法看到图片内容。请用一句话简短收尾即可（例如：资料图已发送，请查看上方图片）。"
        "严禁转述、列出或编造图片中的任何具体数值、属性、材料清单、掉落概率等内容。"
    )
