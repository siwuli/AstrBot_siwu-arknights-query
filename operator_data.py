# -*- coding: utf-8 -*-
"""干员/召唤物数据组装（移植自 amiyabot-arknights-gamedata-4_0/builder/operatorBuilder.py）。

纯 Python 逻辑，无框架依赖；数据源为 gamedata.py 中的 JsonData 内存缓存。
"""

import re

from .gamedata import JsonData, config, html_symbol
from .utils import remove_punctuation, remove_xml_tag, integer
...
class OperatorImpl:
    ...
    def __init__(self, code: str, data: dict, is_recruit: bool = False):
        ...
        self.__skins_list = sorted(
            Collection.get_skins_list(code), key=lambda n: n["displaySkin"]["getTime"]
        )
        ...
        self.type = config["types"].get(data["position"])
        self.tags = []
        self.range = "无范围"
        self.rarity = (
            int(data["rarity"].split("_")[-1])
            if isinstance(data["rarity"], str)
            else (data["rarity"] + 1)
        )
        ...
        self.limit = self.name in config.get("limit", [])
        self.unavailable = self.name in config.get("unavailable", [])
    ...
    def detail(self):
        ...
        if self.data["trait"]:
            ...
            trait = parse_template(
                max_trait["blackboard"], max_trait["overrideDescripton"] or trait
            )
                    ...
                    skills[sk["skillId"]]["levels"] = [
                        {
                            **nn,
                            "description": (
                                parse_template(nn["blackboard"], nn["description"])
                                if nn["description"]
                                else ""
                            ),
                        }
    ...
    def potential(self):
        ...
        if self.data["potentialRanks"]:
            for index, item in enumerate(self.data["potentialRanks"]):
                potential.append(
                    {"potential_desc": item["description"], "potential_rank": index + 1}
                )
        return potential
    ...
    def __tags(self):
        ...
        if str(self.rarity) in config["high_star"]:
            tags.append(config["high_star"][str(self.rarity)])
    ...
    def __cv(self):
        ...
        if self.id in word_data["voiceLangDict"]:
            ...
            self.cv = {
                word_data["voiceLangTypeDict"][name]["name"]: item["cvName"]
...
class TokenImpl:
    ...
    def __init__(self, code: str, data: dict):
        ...
        self.classes = config["token_classes"].get(data["profession"])
        self.type = config["types"].get(data["position"])
        self.attr = []
